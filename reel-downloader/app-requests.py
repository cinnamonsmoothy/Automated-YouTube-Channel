...
class Main:
    ...
    def Download_reel(self, reel_id, file_name, username, password):
        ...
        if not bot.Login(username, password):
            self.Print_error('Bot failed to login')
            self.Quit()
        else: self.Print_success_message('Bot logged in successfully')
            
        # Download
        if not bot.Download_reel(reel_id, file_name): self.Print_error('Bot failed to download reel')
        else: self.Print_success_message('Bot downloaded reel successfully')
